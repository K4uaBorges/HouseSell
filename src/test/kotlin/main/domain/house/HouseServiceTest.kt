...
class HouseServiceTest {
    ...
    @Test
    fun `reject duplicate title for same owner`() {
        val owner = Uuid.random()
        service.createHouse(owner, "Casa Azul", Uuid.random(), 120, 95.0, "Descricao 1")

        val error =
            assertFailsWith<IllegalArgumentException> {
                service.createHouse(owner, "  casa azul  ", Uuid.random(), 90, 80.0, "Descricao 2")
            }

        assertEquals("Já existe uma house tua com esse título.", error.message)
    }

    @Test
    fun `reject duplicate description for same owner`() {
        val owner = Uuid.random()
        service.createHouse(owner, "Casa Azul", Uuid.random(), 120, 95.0, "Descricao repetida")

        val error =
            assertFailsWith<IllegalArgumentException> {
                service.createHouse(owner, "Casa Verde", Uuid.random(), 90, 80.0, "  descricao   repetida ")
            }

        assertEquals("Já existe uma house tua com essa descrição.", error.message)
    }
    ...
    @Test
    fun `update house rejects duplicate title from another house of same owner`() {
        val owner = Uuid.random()
        service.createHouse(owner, "Casa Azul", Uuid.random(), 100, 90.0, "Descricao inicial")
        val second = service.createHouse(owner, "Casa Verde", Uuid.random(), 120, 110.0, "Outra descricao")

        val error =
            assertFailsWith<IllegalArgumentException> {
                service.updateHouse(second.id, "Casa Azul", second.lid, 120, 110.0, "Outra descricao")
            }
