...
class UsersService(
    ...
    fun createUser(
        ...
        val password = Password.of(passwordRaw)
    ...
    fun getUserByEmail(emailRaw: String): User = repo.getByEmail(Email.of(emailRaw).value)

    fun authenticateUser(
        emailRaw: String,
        passwordRaw: String,
    ): User {
        val user = getUserByEmail(emailRaw)
        val attemptedHash = hashPassword(Password.of(passwordRaw))
    ...
    private fun requireEmailAvailable(
        email: String,
        currentUserId: String? = null,
    ) {
        val found =
            runCatching { repo.getByEmail(email) }
                .getOrElse {
