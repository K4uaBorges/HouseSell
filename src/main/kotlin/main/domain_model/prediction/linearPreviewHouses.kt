package main.domain_model.prediction
...
import main.domain_model.house.House as DomainHouse
...
fun loadHousesFromRepository(repository: HouseRepository): List<House> =
    repository.getAll().map { it.toLinearSample() }
...
class Scale(
    ...
    init {
        require(delta != 0.0) {
            "Cannot create Scale: all values are identical (max=$max, min=$min)"
        }
    }
