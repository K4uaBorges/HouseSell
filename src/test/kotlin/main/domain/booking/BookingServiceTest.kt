...
class BookingServiceTest {
    ...
    fun `update booking changes dates`() {
        ...
        val updated = service.updateBooking(booking, "2026-05-11", "2026-05-16")
    ...
    @Test
    fun `list available days returns every day when month has no bookings`() {
        val house = createHouse("Casa Livre")

        val days = service.listAvailableDaysByMonth(house.id, 2026, 2)

        assertEquals(28, days.size)
        assertEquals("2026-02-01", days.first())
        assertEquals("2026-02-28", days.last())
    }

    @Test
    fun `list available days excludes each overlapped day using exclusive end date`() {
        val house = createHouse("Casa Reservada")
        service.createBooking(Uuid.random(), house.id, "2026-05-10", "2026-05-12")

        val days = service.listAvailableDaysByMonth(house.id, 2026, 5)

        assertFalse(days.contains("2026-05-10"))
        assertFalse(days.contains("2026-05-11"))
        assertEquals("2026-05-12", days.first { it >= "2026-05-12" })
    }

    @Test
    fun `list available days rejects invalid month`() {
        val house = createHouse("Casa A")

        assertFailsWith<IllegalArgumentException> {
            service.listAvailableDaysByMonth(house.id, 2026, 13)
        }
    }
